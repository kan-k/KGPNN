...
meanstats.re<- function(filename, runs, mala_if){
  ...
  for(i in runs){
    ...
    dat.in <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_inpred__jobid_',i,'.feather')))) #Taking only the indices and the predictions but not class assignments
    print(dim(dat.in))
    print(head(dat.in))
    if(mala_if){ 
    ...
    #Test
    dat.test <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_outpred__jobid_',i,'.feather'))))
...
#   stat.eb <-meanstats.re("aug9_pm_sm_gpols_12init_sgld", run, mala_if=FALSE)
#   write.csv(stat.eb,paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/summary_re_aug9_pm_sm_gpols_sgld_",run,".csv"))
# }
