...
1. Record the best params on the validation set and use those to make predictions ==> note that this mean I also have to record the hidden layer params, otherwise fitting models on full data will be different.
  modified `pm_sm_gpols_12init_K_bbs.R` oct21_pm_sm_gpols_12init_bbs 18835952
...
colnames(results) <- c("Training RMSE", "Test RMSE" ,"Training R^2", "Test R^2")
rownames(results) <- c("Lasso", "Ridge", "GPR")
...
lasso_quantiles <- apply(hs.int[, c(5, 6,1,3)], MARGIN = 2, FUN = quantile, c(0.5))
results["Lasso", ] <- lasso_quantiles
...
ridge_quantiles <- apply(hs.int[, c(5, 6,1,3)], MARGIN = 2, FUN = quantile, c(0.5))
results["Ridge", ] <- ridge_quantiles
...
gpr_quantiles <- apply(hs.int[, c(5, 6,1,3)], MARGIN = 2, FUN = quantile, c(0.5))
results["GPR", ] <- gpr_quantiles
...
# Reorder columns to put "Method" as the first column
results_df <- results_df[, c("Method", "Training RMSE", "Test RMSE", "Training R^2", "Test R^2")]
