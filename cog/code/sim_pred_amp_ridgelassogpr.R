...
write.csv( c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = ind.to.use$train,ind.new = ind.to.use$test)))),as.numeric(sub('.*:', '', summary(coef(lassofit, s=lassofit$lambda.min)[-1,]))),sum(abs(coef(lassofit, s=lassofit$lambda.min))>1e-8)),
           paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_ridge_noscale_",JobId,".csv"), row.names = FALSE)

print("write prediction")

write.csv(c(pred_prior_new),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_ridge_outpred_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_ridge_inpred_noscale_",JobId,".csv"), row.names = FALSE)
...
write.csv( c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = ind.to.use$train,ind.new = ind.to.use$test)))),as.numeric(sub('.*:', '', summary(coef(lassofit, s=lassofit$lambda.min)[-1,]))),sum(abs(coef(lassofit, s=lassofit$lambda.min))>1e-8)),
           paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_lasso_noscale_",JobId,".csv"), row.names = FALSE)

print("write prediction")

write.csv(c(pred_prior_new),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_lasso_outpred_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_lasso_inpred_noscale_",JobId,".csv"), row.names = FALSE)
...
write.csv(c(unlist(t(as.matrix(rsqcal2(pred_prior$mean,pred_prior_new$mean,ind.old = train.test.ind$train,ind.new = train.test.ind$test)))),as.numeric(sub('.*:', '', summary(lassofit$post_mean$betacoef[-1,]))),sum(abs(lassofit$post_mean$betacoef[-1,])>1e-5)),
          paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior_new$mean),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_outpred_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior$mean),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_inpred_noscale_",JobId,".csv"), row.names = FALSE)
####Result to use
write.csv(rbind(c(train.test.ind$train),c(train.test.ind$test)),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_index_",JobId,".csv"), row.names = FALSE)
write_feather(as.data.frame(lassofit$post_mean$betacoef),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_coef_',JobId,'.feather'))
...
writeNIfTI(gp.mask.nm,paste0('/well/nichols/users/qcv214/KGPNN/cog/viz/sim_oct1_pred_amp_gpr_',poly_degree,a_concentration,b_smoothness,JobId))
...
in.mse <- mean((pred_prior$mean-age_tab$pred_amp[train.test.ind$train])^2)
...
for(i in 1:length(age_tab$pred_amp[train.test.ind$train])){
  within.pred <- c(within.pred,(between(age_tab$pred_amp[train.test.ind$train][i],stat.in.ig.lwrmod[i],stat.in.ig.uprmod[i])))
  within.pred2 <- c(within.pred2,(between(age_tab$pred_amp[train.test.ind$train][i],stat.in.ig.lwrmod2[i],stat.in.ig.uprmod2[i])))
}
...
within.pred <- vector(mode='numeric')
within.pred2 <- vector(mode='numeric')
for(i in 1:length(age_tab$pred_amp[train.test.ind$test])){
  within.pred <- c(within.pred,(between(age_tab$pred_amp[train.test.ind$test][i],stat.out.ig.lwrmod[i],stat.out.ig.uprmod[i])))
  within.pred2 <- c(within.pred2,(between(age_tab$pred_amp[train.test.ind$test][i],stat.out.ig.lwrmod2[i],stat.out.ig.uprmod2[i])))
...
print('Dine')
write.csv(cover.mat,paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_oct1_pred_amp_gpr_coverage_",JobId,".csv"), row.names = FALSE)
print("all done")
