#29 Sep   ##Modelling a realisation of K= 2 GP-OLS, doing K = 2 and 4 ==> this one is modelling pred_amp
...
filename <- "sep29_pm_sm_gpols_pred_amp_bbs" 
success.run <- 1:10
...
################################################################################################################################################
for(num.lat.class.select in c(2,4)){
  ...
  alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_minalpha__jobid_",init.num,".csv"))$x #shape
  beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_minbeta__jobid_",init.num,".csv"))$x #scale
  ...
  #Define init var
  prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG
  
  #Fix prior var to be 0.1
  # prior.var <- 1.5
  y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_minsigma__jobid_",init.num,".csv"))$x
  y.sigma.vec <- y.sigma
  ...
  theta.matrix <- as.matrix(read_feather(paste0( "/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_mintheta__jobid_",init.num,'.feather')))
  co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_mincoweights__jobid_",init.num,".csv")))
  
  #Initialising bias (to 0)
  bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_minbias__jobid_",init.num,".csv"))$x
  co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_sep29_pm_sm_gpols_pred_amp",'_K',num.lat.class.select,"_mincobias__jobid_",init.num,".csv"))$x
  ...
  for(e in 1:epoch){
    ...
    #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/cog/viz/sim_',filename,"_epoch_",e,'_mainsal_',JobId))
    #   #   
    #   #   
    #   #   
    ...
    #   gp.mask.hs@datatype = 16
    #   gp.mask.hs@bitpix = 32
    #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/cog/viz/sim_',filename,"_epoch_",e,'_inter1sal_',JobId))
    #   #   
    ...
    #   gp.mask.hs@datatype = 16
    #   gp.mask.hs@bitpix = 32
    #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/cog/viz/sim_',filename,"_epoch_",e,'_inter2sal_',JobId))
      #   
      ...
      #   gp.mask.hs@datatype = 16
      #   gp.mask.hs@bitpix = 32
      #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/sim_',filename,"_epoch_",e,'_inter3sal_',JobId))
      #   
      ...
      #   gp.mask.hs@datatype = 16
      #   gp.mask.hs@bitpix = 32
      #   writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/sim_',filename,"_epoch_",e,'_inter4sal_',JobId))
      #   
  ...
  write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(rbind(rsq.train,rsq.val),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_rsq_","_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(rbind(loss.train.male,loss.val.male),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_lossM_","_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(rbind(rsq.train.male,rsq.val.male),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_rsqM_","_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(rbind(loss.train.fmale,loss.val.fmale),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_lossF_","_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(rbind(rsq.train.fmale,rsq.val.fmale),paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_rsqF_","_jobid_",JobId,".csv"), row.names = FALSE)
  
  
  write.csv(map.train,paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/sim_",filename,'_K',num.lat.class.select,"_map_","_jobid_",JobId,".csv"), row.names = FALSE)
  write_feather(as.data.frame(theta.matrix),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_theta_',"_jobid_",JobId,'.feather'))
  write.csv(bias,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_bias_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(y.sigma.vec,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_sigma_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(l.bias,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_lbias_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(lr.vec,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_lr_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(pre.lr.vec,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_prelr_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write_feather(as.data.frame(c(beta_fit$HS )),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_lweights_',"_jobid_",JobId,'.feather'))
  write.csv(co.weights,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_coweights_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(co.bias,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_cobias_',"_jobid_",JobId,".csv"), row.names = FALSE)
  ...
  write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_inpred_',"_jobid_",JobId,'.feather'))
  ...
  write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_outpred_',"_jobid_",JobId,'.feather'))
  
  
  #inv gamme param
  write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
  write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/sim_',filename,'_K',num.lat.class.select,'_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)
}