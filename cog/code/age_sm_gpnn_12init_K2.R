...
filename <- "oct14_age_sm_gpnn_12init_fixlr"
# prior.var <- 0.05 #was 0.05
learning_rate <- 0.3#for slow decay starting less than 1
prior.var.bias <- 1
epoch <- 50 #was 400
beta.bb<- 0.5
...
lognum <- as.numeric(age_tab$age) #age_tab$pm_tf
...
quantile_thresholds <- quantile(age_tab$pm_tf[train.test.ind$train], probs = seq(0, 1, by = 0.34))
#age.group <- ifelse(age > mean(age), yes = 1, no = -1)
age.group1 <- ifelse(age_tab$pm_tf <=quantile_thresholds[[2]], yes =1, no = 0)
age.group2 <- ifelse(age_tab$pm_tf > quantile_thresholds[[2]] & age_tab$pm_tf <=quantile_thresholds[[3]], yes =1, no = 0)
age.group3 <- ifelse(age_tab$pm_tf > quantile_thresholds[[3]], yes =1, no = 0)
...
for(num.lat.class.select in c(2)){
  ...
  for(e in 1:epoch){
    ...
    for(b in 1:num.batch){
      ...
      map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mseCpp(hs_in.pred_SOI,lognum[mini.batch$train[[b]]]) +n.mask*p.dat/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) +l.expan/2*log(y.sigma) + 1/(2*y.sigma)*(1/1)*sum((hs_fit_SOI$post_mean$betacoef[2:(l.expan+1)])^2) + 1/2*l.bias^2)
  ...
  write_feather(as.data.frame(weights),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_K',num.lat.class.select,'_weights_',"_jobid_",JobId,'.feather'))
  ...
  write_feather(as.data.frame(min.weights),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_K',num.lat.class.select,'_minweights_',"_jobid_",JobId,'.feather'))
  write_feather(as.data.frame(min.gp.coef),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_K',num.lat.class.select,'_minLCgp_',"_jobid_",JobId,'.feather'))
