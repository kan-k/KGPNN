...
filename <- "oct30_pm_sm_gpnn_12init_sgld_lr99_run4"
success.run <- c(1:10)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minalpha__jobid_",4,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minbeta__jobid_",4,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minpriorvar__jobid_",4,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minsigma__jobid_",4,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minweights__jobid_',4,'.feather')))

#jitter
rand.noise <- median(abs(c(weights)))^2
print(paste0("added noise = ", rand.noise))
weights <- weights -  matrix(rnorm(p.dat*n.mask,0,sqrt(rand.noise)), ncol = p.dat, nrow = n.mask) 


co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_mincoweights__jobid_",4,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minbias__jobid_",4,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_mincobias__jobid_",4,".csv"))$x

num.lat.class<- length(co.bias)

#non-imaging covar
gp.coef <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct30_pm_sm_gpnn_12init_minLCgp__jobid_',4,'.feather')))
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    if(it.num < epoch*num.batch){
      ...
      #Update theta matrix
      weights <- weights*(1-learning_rate*1/(prior.var*y.sigma)) - learning_rate*grad.m * length(train.test.ind$train) - matrix(rnorm(n.mask*p.dat,0,gaus.sd), ncol = p.dat, nrow = n.mask)
      ...
      gp.coef <- gp.coef*(1-learning_rate) - learning_rate*gp.coef.grad.m* n.train -  matrix(rnorm(num.lat.class*l.expan,0,gaus.sd),nrow = num.lat.class )
...
# colnames(temp.frame) <- NULL
# colnames(temp.frame) <- 1:ncol(temp.frame)
# write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_min_outpred_ext_',"_jobid_",JobId,'.feather'))
