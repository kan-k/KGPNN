...
filename <-  "oct31_pm_sm_gpgp_12init_sgld_lr99_run9"
success.run <- c(1:10)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minalpha__jobid_",9,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minbeta__jobid_",9,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minpriorvar__jobid_",9,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minsigma__jobid_",9,".csv"))$x
y.sigma.vec <- y.sigma
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_mintheta__jobid_',9,'.feather')))
# theta.matrix <- theta.matrix -  matrix(rnorm(n.mask*n.expan,0,sqrt(learning_rate*2)), ncol = n.expan, nrow = n.mask)
rand.noise <- median(abs(c(theta.matrix)))^2
print(paste0("added noise = ", rand.noise))
theta.matrix <- theta.matrix -  matrix(rnorm(n.mask*n.expan,0,sqrt(rand.noise)), ncol = n.expan, nrow = n.mask)

co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_mincoweights__jobid_",9,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minbias__jobid_",9,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_mincobias__jobid_",9,".csv"))$x

num.lat.class<- length(co.bias)

#non-imaging covar
gp.coef <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_oct31_pm_sm_gpgp_12init_minLCgp__jobid_',9,'.feather')))
...
# write_feather(as.data.frame(min.theta),paste0( '/well/nichols/users/qcv214/KGPNN/cog/pile/re_',filename,'_minweights_',"_jobid_",JobId,'.feather'))
