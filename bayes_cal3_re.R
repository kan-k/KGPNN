...
meanstats.re<- function(filename, runs, mala_if){
  ...
  train.test.ind$train <-  read.csv('/well/nichols/users/qcv214/KGPNN/sex_train_index.csv')$x #[1:200] only active if it is mala or sgld200
  ...
  # res.mat<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_",filename,"_loss__jobid_",runs,".csv")))
  # mod.mse <- mean(res.mat[1,])
  
  for(i in runs){
    print(paste0("run: ",i))
    res.mat<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_",filename,"_loss__jobid_",i,".csv")))
    mod.mse <- mean(res.mat[1,])
    dat.in <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_inpred__jobid_',i,'.feather'))))[,1:3] #Taking only the indices and the predictions but not class assignments
    print(dim(dat.in))
    print(head(dat.in))
    if(mala_if){ 
    ...
    #Test
    dat.test <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_outpred__jobid_',i,'.feather'))))[,1:3]
