...
dimension = 3
deg_vec<- n.expan.degree #was 6 before 25 Apr 24
n.expan <- choose(deg_vec+3,3)
a_vec  <- c(1,2,3,4,5)
...
tf.res3.dat <- array( , dim =c(n.mask,n.dat,n.expan))
...
for(i in res3.mask.reg){
  ...
  #####
    #res3.dat is (n x p)
    #bases.nb.to.use is (p x l)
  tf.res3.dat[which(i==res3.mask.reg),,] <- as.matrix(res3.dat %*% bases.nb.to.use) #the which() overcomes the issue of non-consec region numbers
  #####
}

res3.dat <- tf.res3.dat
rm(tf.res3.dat)
