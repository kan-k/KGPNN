...
meanstats.re<- function(filename, runs, mala_if){
  ...
  train.test.ind$train <-  read.csv('/well/nichols/users/qcv214/KGPNN/sim15_train_index.csv')$x #[1:200] only active if it is mala or sgld200
  ...
  #add class
  true.train<- data.frame(train.test.ind$train,train.true, age_tab$class[train.test.ind$train],row.names = NULL)
  colnames(true.train) <- c('id','true','class')

  test.true <- age[train.test.ind$test]
  true.test<- data.frame(train.test.ind$test,test.true, age_tab$class[train.test.ind$test],row.names = NULL)
  colnames(true.test) <- c('id','true','class')
  
  num.it <-2000 #500 epochs, 4 minibatches
  

  #This does not seem to work on 26 May. I will change it to indidual runs instead
  # res.mat<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_loss__jobid_",runs,".csv")))
  # mod.mse <- mean(res.mat[1,])
  
  for(i in runs){
    print(paste0("run: ",i))
    res.mat<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_loss__jobid_",i,".csv")))
    mod.mse <- mean(res.mat[1,])
    dat.in <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_inpred__jobid_',i,'.feather'))))[,1:3] #Taking only the indices and the predictions but not class assignments
    print(dim(dat.in))
    print(head(dat.in))
    if(mala_if){ 
    ...
    colnames(joined_data.train) <- c("subject_id", "truth","true_class","mean_pred", "lwr_ppi", "upr_ppi","pred_class", "within_interval")
    ...
    #Test
    dat.test <- as.data.frame(t(read_feather(paste0('/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_outpred__jobid_',i,'.feather'))))[,1:3]
    ...
    joined_data.test <- joined_data.test %>%
      mutate(within_interval = ifelse(true>= lwr_ppi & true <= upr_ppi, TRUE, FALSE))
    colnames(joined_data.test) <- c("subject_id", "truth","true_class","mean_pred", "lwr_ppi", "upr_ppi","pred_class", "within_interval")
  ...
  out <- c(out,unlist(result.train),unlist(result.test), adjustedRandIndex(joined_data.train$true_class,joined_data.train$pred_class), adjustedRandIndex(joined_data.test$true_class,joined_data.test$pred_class))
...
success.run <- c(1:4,6:10)
for(run in success.run){
  stat.eb <-meanstats.re("june1_sm_depind15_gpols_once_init_sgld", run, mala_if=FALSE)
  write.csv(stat.eb,paste0("/well/nichols/users/qcv214/KGPNN/pile/summary_sim_june1_sm_depind15_gpols_once_init_sgld_",run,".csv"))
}
