...
filename <- "jan28_lowhid_gpnn_bbig_init_bbs" 
success.run <- 1:10
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minweights__jobid_',init.num,'.feather')))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan27_lowhid_gpnn_bbig_init_minbias__jobid_",init.num,".csv"))$x
  ...
  for(b in 1:num.batch){
    ...
    if(it.num < epoch*num.batch){
      ...
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad[,j,] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j])*c(relu.prime(hidden.layer[,j]))*res3.dat[mini.batch$train[[b]], ]  #######this is wrong shttt. sex shouldnt be there, it should be the sigmoid
      }
      #Take batch average
      ...
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad.b[,j] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j] #r1c4
