...
filename <- "july3_sm_depind_gpnn_init_bbs" 
success.run <- c(1,6,9,10)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minweights__jobid_',init.num,'.feather')))
co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_mincoweights__jobid_",init.num,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minbias__jobid_",init.num,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_mincobias__jobid_",init.num,".csv"))$x

num.lat.class<- length(co.bias)

#non-imaging covar
gp.coef <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_july3_sm_depind_gpnn_init_minLCgp__jobid_',init.num,'.feather')))
