...
#Jan 5: use bot and top 15% of deprivation index
...
filename <- "jan18_sm_depind15_lovar_ysigcap_gpnn_bbig_init"
...
#Get minibatch index 
batch_size <- 237 #Correspondds to 4 batches 500,500,500,471
...
alpha.init <- rep(101,n.mask) #shape 
beta.init <- rep(0.01,n.mask) #scale
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    if(it.num < epoch*num.batch){
      ...
      print('summary(c(weights))')
      print(summary(c(weights)))
      print("summary(c(prior.var))")
      print(summary(c(prior.var)))
      print("y.sigma")
      print(y.sigma)
      ...
      ################# capping y.sigma at 100 and abs(y.sigma) to avoid negative sigma
      y.sigma <- min(abs(y.sigma),100) 
      #################
