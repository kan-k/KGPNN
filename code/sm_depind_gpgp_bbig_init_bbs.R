...
filename <- "dec28_sm_depind_gpgp_bbig_init_bbs" 
success.run <- c(1:5,7:10)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_mintheta__jobid_',init.num,'.feather')))
co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_mincoweights__jobid_",init.num,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_minbias__jobid_",init.num,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec27_sm_depind_gpgp_bbig_init_mincobias__jobid_",init.num,".csv"))$x
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    if(it.num < epoch*num.batch){
      ...
      for(j in 1:n.mask){ #nrow of weights = n.mask
        ...
        )*c(relu.prime(hidden.layer[,j]))*res3.dat[j,mini.batch$train[[b]], ]  #######this is wrong shttt. sex shouldnt be there, it should be the sigmoid
      }
      #Take batch average
  ...
  if(e %in% c(epoch)){ #300 and 500 #save last epoch
    ...
    salient.mat <- t(t(beta_fit$HS[n.mask+num.lat.class+1+(seq(n.mask)-1)*num.lat.class]*weights) %*% t(c(co.hidden.layer[,1]) * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    ...
    salient.mat <- t(t(beta_fit$HS[n.mask+num.lat.class+2+(seq(n.mask)-1)*num.lat.class]*weights) %*% t(c(co.hidden.layer[,2]) * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    ...
    salient.mat <- t(t(beta_fit$HS[n.mask+num.lat.class+3+(seq(n.mask)-1)*num.lat.class]*weights) %*% t(c(co.hidden.layer[,3]) * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    ...
    salient.mat <- t(t(beta_fit$HS[n.mask+num.lat.class+4+(seq(n.mask)-1)*num.lat.class]*weights) %*% t(c(co.hidden.layer[,4]) * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
