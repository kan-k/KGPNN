...
filename <- "may1_sm_depind15_gpols_bbig_init_min" 
success.run <- c(1,2,3,4,6,7,9)
init.num <- ifelse(JobId %in% success.run, yes = JobId, no = FALSE)
# init.num <- 10
...
hidden.layer <- apply(t(apply(res3.dat[, train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu) #not C++ optim, this collapes into two dimension
...
z.nb <- cbind(poly_features, co.hidden.layer, interaction_features)

fit.lm <- lm(age[train.test.ind$train] ~ z.nb)
...
map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mseCpp(hs_in.pred_SOI,age[train.test.ind$train]) +n.mask/2*log(y.sigma) +n.mask*n.expan/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) )
