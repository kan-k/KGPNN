...
filename <- "may1_sm_depind15_gpols_bbig_init_bbs" 
...
print(paste0("load up res4 gp [DONE]"))
print(paste0("dim res4 gp"))
print(dim(res3.dat))
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_mintheta__jobid_',init.num,'.feather')))
co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_mincoweights__jobid_",init.num,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_minbias__jobid_",init.num,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_may1_sm_depind15_gpols_bbig_init_mincobias__jobid_",init.num,".csv"))$x
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mseCpp(hs_in.pred_SOI,age[mini.batch$train[[b]]]) +n.mask/2*log(y.sigma) +n.mask*n.expan/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) )
...
#inv gamme param
write.csv(conj.alpha,paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_alpha_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta,paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_beta_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma,paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_invgam_',"_jobid_",JobId,".csv"), row.names = FALSE)