...
filename <- "apr21_sm_depind15_mixed_gpnn_bbig_init"
...
#Initial parameters for inverse gamma
alpha.init <-  min.alpha
beta.init <-  min.beta

#Define init var
prior.var <-  min.prior.var
...
weights <- as.data.frame(min.weights)
co.weights<- min.co.weights
...
partial.gp.centroid <- rbind(partial.gp.centroid.img,as.data.frame(min.gp.coef))
...
loss.train <- mseCpp(hs_in.pred_SOI,age[train.test.ind$train])
rsq.train <- rsqCpp(age[train.test.ind$train],hs_in.pred_SOI)

loss.train.male <- mseCpp(hs_in.pred_SOI[which(sex[train.test.ind$train] == 1)],age[train.test.ind$train][which(sex[train.test.ind$train] == 1)])
rsq.train.male <- rsqCpp(age[train.test.ind$train][which(sex[train.test.ind$train] == 1)],hs_in.pred_SOI[which(sex[train.test.ind$train] == 1)])

loss.train.fmale <- mseCpp(hs_in.pred_SOI[which(sex[train.test.ind$train] == -1)],age[train.test.ind$train][which(sex[train.test.ind$train] == -1)])
rsq.train.f <- rsqCpp(age[train.test.ind$train][which(sex[train.test.ind$train] == -1)],hs_in.pred_SOI[which(sex[train.test.ind$train] == -1)])

temp.sum.sum.sq <- apply(weights, 1, FUN = function(x) sum(x^2))

#Note wrong MAP here. I have NOT incorporated intercept
map.train <- n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mseCpp(hs_in.pred_SOI,age[train.test.ind$train]) +n.mask/2*log(y.sigma) +n.mask*p.dat/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) 
...
loss.val <- mseCpp(hs_pred_SOI,age[train.test.ind$test])
rsq.val <- rsqCpp(age[train.test.ind$test],hs_pred_SOI)

loss.val.male <-  mseCpp(hs_pred_SOI[which(sex[train.test.ind$test] == 1)],age[train.test.ind$test][which(sex[train.test.ind$test] == 1)])
rsq.val.male <- rsqCpp(age[train.test.ind$test][which(sex[train.test.ind$test] == 1)],hs_pred_SOI[which(sex[train.test.ind$test] == 1)])

loss.val.fmale <- mseCpp(hs_pred_SOI[which(sex[train.test.ind$test] == -1)],age[train.test.ind$test][which(sex[train.test.ind$test] == -1)])
rsq.val.fmale <- rsqCpp(age[train.test.ind$test][which(sex[train.test.ind$test] == -1)],hs_pred_SOI[which(sex[train.test.ind$test] == -1)])
...
pred.train.ind <- train.test.ind$train
pred.train.val <- hs_in.pred_SOI
pred.test.ind <- train.test.ind$test
pred.test.val <- hs_pred_SOI
#class
class.train.val <- apply(co.hidden.layer,1,which.max) 
class.test.val <- apply(co.hidden.layer.test,1,which.max) 

time.taken <- Sys.time() - time.train
cat("Training complete in: ", time.taken)
write.csv(rbind(loss.train,loss.val),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_loss_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(rsq.train,rsq.val),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_rsq_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(loss.train.male,loss.val.male),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_lossM_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(rsq.train.male,rsq.val.male),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_rsqM_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(loss.train.fmale,loss.val.fmale),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_lossF_","_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(rbind(rsq.train.fmale,rsq.val.fmale),paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_",filename,"_min_rsqF_","_jobid_",JobId,".csv"), row.names = FALSE)

temp.frame <- as.data.frame(rbind(pred.train.ind,pred.train.val,class.train.val))
colnames(temp.frame) <- NULL
colnames(temp.frame) <- 1:ncol(temp.frame)
# temp.frame<- t(tail(t(temp.frame),length(train.test.ind$train)*5))
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_min_inpred_',"_jobid_",JobId,'.feather'))
temp.frame <- as.data.frame(rbind(pred.test.ind,pred.test.val,class.test.val))
colnames(temp.frame) <- NULL
colnames(temp.frame) <- 1:ncol(temp.frame)
# temp.frame<- t(tail(t(temp.frame),length(train.test.ind$test)*5))
write_feather(temp.frame,paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_',filename,'_min_outpred_',"_jobid_",JobId,'.feather'))
