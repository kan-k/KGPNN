...
filename <- "feb1_bimodal_gpnn_bbig_init_bbs" 
success.run <- 1:10
...
##########
print("weights1")
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minweights__jobid_',init.num,'.feather')))
print("weights2")
weights.dmn<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minweightsdmn__jobid_",init.num,".csv")))
...
partial.gp.centroid<-t(as.matrix(read_feather(paste0("/well/nichols/users/qcv214/KGPNN/partial_gp_centroids_fixed_10.540.feather")))) #12 x 4
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
print("alpha-beta2")

alpha.init.dmn <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minalphadmn__jobid_",init.num,".csv"))$x #shape
beta.init.dmn <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minbetadmn__jobid_",init.num,".csv"))$x #scale
...
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG
print("prior var2")

prior.var.dmn <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minpriorvardmn__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
print("sigma")

y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
print("bias1")
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minbias__jobid_",init.num,".csv"))$x
print("bias2")

bias.dmn <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_jan31_bimodal_gpnn_bbig_init_minbiasdmn__jobid_",init.num,".csv"))$x


prior.var.bias <- 1
prior.var.bias.dmn <- 1
print("bias completed")
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    ############################################################################################
    #####This is the thing, I don't want interactions between all. I only want between ths same regions => the problem is the sense of regions is lost from hidden.features
    #But anyway this is like timing degree of expansion. 
    interaction_features <- sapply(1:ncol(hidden.dmn.features), function(i) {
      elementwise_product(hidden.dmn.features[, i], hidden.features[, i])
    })
    # Create the design matrix
    interaction_features <- array(data = interaction_features, dim = c(nrow(hidden.dmn.features), ncol(hidden.dmn.features)))
    
    z.nb <- cbind(1,hidden.features, hidden.dmn.features, interaction_features) #This is different from LASIR in the sense that the subgroup latent directly affect the output, whereas the group themselves dont. But then that can be modified easily.
    ...
    #####This is the thing, I don't want interactions between all. I only want between ths same regions => the problem is the sense of regions is lost from hidden.features
    #But anyway this is like timing degree of expansion. 
    interaction_features.test <- sapply(1:ncol(hidden.dmn.features.test), function(i) {
      elementwise_product(hidden.dmn.features.test[, i], hidden.features.test[, i])
    })
    # Create the design matrix
    interaction_features.test <- array(data = interaction_features.test, dim = c(nrow(hidden.dmn.features.test), ncol(hidden.dmn.features.test)))
    
    z.nb.test <- cbind(1,hidden.features.test, hidden.dmn.features.test, interaction_features.test) #This is different from LASIR in the sense that the subgroup latent directly affect the output, whereas the group themselves dont. But then that can be modified easily.
    ...
    if(it.num < epoch*num.batch){
      ...
      grad.dmn <- array(,dim = c(minibatch.size,dim(weights.dmn)))
      
      #Each 
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad[,j,] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j]+
                                                beta_fit$HS[j+n.mask*2]*c(hidden.layer.dmn[,j])
        )*c(relu.prime(hidden.layer[,j]))*res3.dat[mini.batch$train[[b]], ]  
        
        grad.dmn[,j,] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j+n.mask]+
                                                    beta_fit$HS[j+n.mask*2]*c(hidden.layer[,j])
        )*c(relu.prime(hidden.layer.dmn[,j]))*res3.dat.dmn[mini.batch$train[[b]], ]
      }
      #Sex shouldnt be in the above, it should be sigmoid. That means all of my sigmoid are wrong.
      #Take batch average
      grad.m <- apply(grad, c(2,3), mean)
      grad.dmn.m <- apply(grad.dmn, c(2,3), mean)
      
      
      #####
      #########Here is inefficiency
      grad.b <- matrix(, nrow = minibatch.size, ncol = length(bias))
      grad.b.dmn <- matrix(, nrow = minibatch.size, ncol = length(bias.dmn))
      
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad.b[,j] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j]+
                                                 beta_fit$HS[j+n.mask*2]*c(hidden.layer.dmn[,j])#r1c4
        )*c(relu.prime(hidden.layer[,j]))  
        
        grad.b.dmn[,j] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j+n.mask]+
                                                     beta_fit$HS[j+n.mask*2]*c(hidden.layer[,j])#r1c4
        )*c(relu.prime(hidden.layer.dmn[,j])) 
      }
      #Take batch average
  ...
  if(e %in% c(epoch)){ #300 and 500
    ...
    salient.mat <- t(t(beta_fit$HS[(n.mask+1):(n.mask*2)]*weights.dmn) %*% t(apply(t(t(res3.dat.dmn[train.test.ind$train, ]  %*% t(weights.dmn)) + bias.dmn), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/',filename,"_epoch_",e,'_maindmnsal_',JobId))
    
    ########It depends on what saliency we are looking at..... I will think about it later.
    salient.mat <- t(t(beta_fit$HS[(n.mask*2+1):(n.mask*3)]*weights) %*% t(hidden.layer.dmn * apply(t(t(res3.dat[mini.batch$train[[b]], ]  %*% t(weights)) + bias), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    ...
    salient.mat <- t(t(beta_fit$HS[(n.mask*2+1):(n.mask*3)]*weights.dmn) %*% t(hidden.layer * apply(t(t(res3.dat.dmn[mini.batch$train[[b]], ]  %*% t(weights.dmn)) + bias.dmn), 2, FUN = relu.prime)))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(salient.mat))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
...
write.csv(weights.dmn,paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_weightsdmn_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(bias.dmn,paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_biasdmn_',"_jobid_",JobId,".csv"), row.names = FALSE)
...
#inv gamme param
write.csv(conj.alpha.dmn,paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_alphadmn_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.beta.dmn,paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_betadmn_',"_jobid_",JobId,".csv"), row.names = FALSE)
write.csv(conj.invgamma.dmn,paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_invgamdmn_',"_jobid_",JobId,".csv"), row.names = FALSE)
