...
filename <- "oct11_gender_nn_bbig_init_bbs"
init.num <- JobId 
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minweights__jobid_',init.num,'.feather')))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_oct2_gender_nn_bbig_init_minbias__jobid_",init.num,".csv"))$x
time.train <-  Sys.time()
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    beta_fit <- data.frame(HS = c(hs_fit_SOI$post_mean$betacoef[2:(n.mask+1)],
                                  hs_fit_SOI$post_mean$betacoef[n.mask+2],
                                  hs_fit_SOI$post_mean$betacoef[-1:-(n.mask+2)] ))
...
write_feather(as.data.frame(c(hs_fit_SOI$post_mean$betacoef[2:(n.mask+1)],
                              hs_fit_SOI$post_mean$betacoef[n.mask+2],
                              hs_fit_SOI$post_mean$betacoef[-1:-(n.mask+2)] )),paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_lweights_',"_jobid_",JobId,'.feather'))
