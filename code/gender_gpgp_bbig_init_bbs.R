...
filename <- "oct30_gender_gpgp_bbig_init_bbs"
init.num <- JobId
...
mult.dat <- function(X) res3.dat %*% X
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
theta.matrix <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_mintheta__jobid_',init.num,'.feather')))
# weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minweights__jobid_',init.num,'.feather')))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_sep29_gender_gpgp_bbig_init_minbias__jobid_",init.num,".csv"))$x
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    #Hidden layer
    ######
    hidden.layer <- apply(t(apply(res3.dat[, mini.batch$train[[b]], ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu)
    ...
    temp.sum.sum.sq.l <- sum(hs_fit_SOI$post_mean$betacoef[-1]^2)
    
    #Note wrong MAP here. I have NOT incorporated intercept
    map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mse(hs_in.pred_SOI,age[mini.batch$train[[b]]]) +l.expan/2*log(y.sigma) + 1/(2*y.sigma)*1/1*(temp.sum.sum.sq.l)  +1/2*l.bias^2 +n.mask*n.expan/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) )
    ...
    if(it.num < epoch*num.batch){
      ...
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad[,j,] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j]+beta_fit$HS[j+n.mask+1]*sex[mini.batch$train[[b]]])*c(relu.prime(hidden.layer[,j]))*res3.dat[j,mini.batch$train[[b]], ]
      }
      #Take batch average
  ...
  # if(e == 2){
    ...
    salient.mat <- t(t(beta_fit$HS[1:n.mask]*weights) %*% t(apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime))) + t(t(beta_fit$HS[(n.mask+2):(n.mask*2+1)]*weights) %*% t(sex[train.test.ind$train] * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
    #Look at the dimensions of things
    # print('Dim weights')
    # print(dim(weights))
    # print('length beta main effects')
    # print(length(beta_fit$HS[1:n.mask]))
    # print('length beta interaction')
    # print(length(beta_fit$HS[(n.mask+2):(n.mask*2+1)]))
    # print('transpose of product of beta*weight')
    # print(dim(t(beta_fit$HS[1:n.mask]*weights)))
    # print('transpose of derivative of relu')
    # print(dim(t(apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime))))
    # 
    # print("Dim of salient matrices of all training subjects, expected n x p (19XX x 150k)")
    # print(dim(salient.mat))
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(t(t(beta_fit$HS[1:n.mask]*weights) %*% t(apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/',filename,"_beta_epoch_",e,'_sal_',JobId))
    
    gp.mask.hs <- res3.mask
    gp.mask.hs[gp.mask.hs!=0] <- abs(colMeans(t(t(beta_fit$HS[(n.mask+2):(n.mask*2+1)]*weights) %*% t(sex[train.test.ind$train] * apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))))
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/',filename,"_interact_epoch_",e,'_sal_',JobId))
    ...
    gp.mask.hs@datatype = 16
    gp.mask.hs@bitpix = 32
    writeNIfTI(gp.mask.hs,paste0('/well/nichols/users/qcv214/KGPNN/viz/',filename,"_epoch_",e,'_sal_',JobId))
  }
