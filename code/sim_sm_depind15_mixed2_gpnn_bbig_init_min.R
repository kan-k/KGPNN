...
filename <- "apr21_sm_depind15_mixed_gpnn_bbig_init_min" 
success.run <- c(1:10)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale



#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minweights__jobid_',init.num,'.feather')))
co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_mincoweights__jobid_",init.num,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minbias__jobid_",init.num,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_mincobias__jobid_",init.num,".csv"))$x

num.lat.class<- length(co.bias)

gp.coef <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/sim_apr21_sm_depind15_mixed_gpnn_bbig_init_minLCgp__jobid_',init.num,'.feather')))
...
map.train <- c(map.train,n.train/2*log(y.sigma) +1/(2*y.sigma)*n.train*mseCpp(hs_in.pred_SOI,age[train.test.ind$train]) +n.mask/2*log(y.sigma) +n.mask*p.dat/2*log(y.sigma) + 1/(2*y.sigma)*sum(1/prior.var*(temp.sum.sum.sq))  +1/2*sum(c(bias)^2) )
