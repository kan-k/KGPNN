...
ind.to.use <- list()
ind.to.use$test <- read.csv('/well/nichols/users/qcv214/KGPNN/sex_test_index.csv')$x
ind.to.use$train <- read.csv('/well/nichols/users/qcv214/KGPNN/sex_train_index.csv')$x
...
write.csv( c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = ind.to.use$train,ind.new = ind.to.use$test)))),as.numeric(sub('.*:', '', summary(coef(lassofit, s=lassofit$lambda.min)[-1,]))),sum(abs(coef(lassofit, s=lassofit$lambda.min))>1e-8)),
           paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_ridge_decind_noscale_",JobId,".csv"), row.names = FALSE)

print("write prediction")

write.csv(c(pred_prior_new),paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_ridge_decind_outpred_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior),paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_ridge_decind_inpred_noscale_",JobId,".csv"), row.names = FALSE)
...
write.csv( c(unlist(t(as.matrix(rsqcal2(pred_prior,pred_prior_new,ind.old = ind.to.use$train,ind.new = ind.to.use$test)))),as.numeric(sub('.*:', '', summary(coef(lassofit, s=lassofit$lambda.min)[-1,]))),sum(abs(coef(lassofit, s=lassofit$lambda.min))>1e-8)),
           paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_lasso_decind_noscale_",JobId,".csv"), row.names = FALSE)

print("write prediction")

write.csv(c(pred_prior_new),paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_lasso_decind_outpred_noscale_",JobId,".csv"), row.names = FALSE)
write.csv(c(pred_prior),paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec19_lasso_decind_inpred_noscale_",JobId,".csv"), row.names = FALSE)