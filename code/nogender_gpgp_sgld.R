...
filename <- "oct2_nogender_gpgp_sgld_a4_b0_near" 
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    poly_features <- as.matrix(hidden.layer %*% partial.gp.centroid)# Create the design matrix
    z.nb <- cbind(1,poly_features)
    ...
    if(e >= (epoch-record.epoch)){
      ...
      if(e==(epoch-record.epoch+1)){ #Added +1 here
        mean.num <- 1
        mean.theta.matrix <- theta.matrix
        mean.bias <- bias
        
        mean.abs.theta.matrix <- abs(theta.matrix)
        mean.abs.bias <- abs(bias)
        
      }else{
        mean.theta.matrix <- (mean.abs.theta.matrix*mean.num + theta.matrix)/(mean.num+1)
        mean.bias <- (mean.abs.bias*mean.num + bias)/(mean.num+1)
        #Mean Magnitude
        mean.abs.theta.matrix <- (mean.abs.theta.matrix*mean.num + abs(theta.matrix))/(mean.num+1)
    ...
    if(it.num < epoch*num.batch){
      ...
      for(j in 1:n.mask){ #nrow of weights = n.mask
        grad[,j,] <- -1/y.sigma*c(grad.loss)*(beta_fit$HS[j])*c(relu.prime(hidden.layer[,j]))*res3.dat[j,mini.batch$train[[b]], ]
      }
      ...
      grad.b <- 1/y.sigma* c(grad.loss)*t(beta_fit$HS[1:n.mask]*t(apply(hidden.layer, 2, FUN = relu.prime))) 
      #Take batch average
      ...
      theta.matrix <- theta.matrix*(1-learning_rate*1/(prior.var*y.sigma)) - learning_rate*grad.m * length(train.test.ind$train) - matrix(rnorm(nrow(theta.matrix)*ncol(theta.matrix),0,gaus.sd), ncol = ncol(theta.matrix), nrow = nrow(theta.matrix))
    ...
    grad.select[,b] <- c(c((theta.matrix/(prior.var*y.sigma) - grad.m * length(train.test.ind$train))[,1]),c(bias/(prior.var.bias) - c(grad.b.m) * length(train.test.ind$train)))
...
write_feather(as.data.frame(c(partial.gp.centroid%*%hs_fit_SOI$post_mean$betacoef[2:(l.expan+1)])),paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_lweights_',"_jobid_",JobId,'.feather'))
...
#Mean parameters
write_feather(as.data.frame(mean.theta.matrix),paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_meantheta_',"_jobid_",JobId,'.feather'))
...
#Mean absparameters
write_feather(as.data.frame(mean.abs.theta.matrix),paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_',filename,'_meanabstheta_',"_jobid_",JobId,'.feather'))
