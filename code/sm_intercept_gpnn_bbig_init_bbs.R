...
filename <- "dec16_sm_intercept_gpnn_bbig_init_bbs"
success.run <- c(1,2,6)
...
alpha.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minalpha__jobid_",init.num,".csv"))$x #shape
beta.init <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minbeta__jobid_",init.num,".csv"))$x #scale
...
#Define init var
prior.var <-  read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minpriorvar__jobid_",init.num,".csv"))$x#Mean of IG

#Fix prior var to be 0.1
# prior.var <- 1.5
y.sigma <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minsigma__jobid_",init.num,".csv"))$x
y.sigma.vec <- y.sigma
...
weights <- as.matrix(read_feather(paste0( '/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minweights__jobid_',init.num,'.feather')))
# co.weights<- as.matrix(read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_mincoweights__jobid_",init.num,".csv")))

#Initialising bias (to 0)
bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_minbias__jobid_",init.num,".csv"))$x
co.bias <- read.csv(paste0("/well/nichols/users/qcv214/KGPNN/pile/re_dec15_sm_intercept_gpnn_bbig_init_mincobias__jobid_",init.num,".csv"))$x
...
for(e in 1:epoch){
  ...
  for(b in 1:num.batch){
    ...
    co.pre.hidden.layer <- matrix(co.bias,nrow=minibatch.size,ncol=length(co.bias),byrow = TRUE) #Doesn't this make everyone identical?
    ...
    co.hidden.layer.test <- softmax(matrix(co.bias,nrow=length(train.test.ind$test),ncol=length(co.bias),byrow = TRUE))
    ...
    if(it.num < epoch*num.batch){
      ...
      delta_f <- c(c(weights/(prior.var*y.sigma) + grad.m*n.train),c(bias/prior.var.bias + grad.b.m*(n.train)),c(co.bias + co.grad.b.m*n.train))
      grad_x <- beta.bb*delta_f + (1-beta.bb)*grad_x
      x.param <- c(c(weights),c(bias),c(co.bias))
      #Update Cv
      for(i in 1:n.mask){
