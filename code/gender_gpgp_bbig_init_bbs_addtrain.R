...
filename <- "oct11_gender_gpgp_bbig_init_bbs_addtrain4k_167"
init.num <- JobId
...
age_tab<- rbind(age_tab,(read_feather('/well/nichols/users/qcv214/KGPNN/age_sex_add1.feather'))[1:num.add,])
...
train.test.ind$train.original <-  read.csv('/well/nichols/users/qcv214/KGPNN/sex_train_index.csv')$x
...
#Storing inv gamma
conj.alpha <- matrix(, nrow=n.mask,ncol=epoch*num.minibatch)
conj.beta <- matrix(, nrow=n.mask,ncol=epoch*num.minibatch)
conj.invgamma <-matrix(, nrow=n.mask,ncol=epoch*num.minibatch)
...
for(e in 1:epoch){
  ...
  if(e %in% c(epoch/2,epoch)){
    ...
    salient.mat <- t(t(beta_fit$HS*weights) %*% t(apply(t(apply(res3.dat[,train.test.ind$train, ],MARGIN = 2,FUN = mult.thea,thet=theta.matrix) + bias), 2, FUN = relu.prime)))
